...
#   stopifnot(
#     "tag must be a single non-NA character string" = 
#       is.character(tag) && length(tag) == 1 && !is.na(tag),
#     "repo must be a single non-NA character string" = 
#       is.character(repo) && length(repo) == 1 && !is.na(repo),
#     "archive_dir must be a single non-NA character string" = 
#       is.character(archive_dir) && length(archive_dir) == 1 && !is.na(archive_dir)
#   )
...
#       # Filter to existing files before upload
#       existing_files <- pb_files[file.exists(pb_files)]
      
#       # Upload each file individually with explicit type conversion
#       for (f in existing_files) {
#         tryCatch({
#           # Ensure single scalar strings
#           file_scalar <- as.character(f)[1]
#           repo_scalar <- as.character(repo)[1]
#           tag_scalar <- as.character(tag)[1]
...
#           )
#         }, error = function(e) {
#           cli::cli_alert_danger("Failed to upload {basename(f)}: {e$message}")
#         })
#       }
#     })
#     # Per-season files
...
#     for (filelist in season_files) {
#       # Extract and ensure scalar strings BEFORE tryCatch blocks
#       pq_scalar <- as.character(filelist$parquet)[1]
#       rds_scalar <- as.character(filelist$rds)[1]
#       csv_scalar <- as.character(filelist$csv)[1]
#       repo_scalar <- as.character(repo)[1]
#       tag_scalar <- as.character(tag)[1]
...
#           )
#         }, error = function(e) {
#           cli::cli_alert_danger("Failed to upload parquet: {e$message}")
#         })
...
#           )
#         }, error = function(e) {
#           cli::cli_alert_danger("Failed to upload rds: {e$message}")
#         })
...
#           )
#         }, error = function(e) {
#           cli::cli_alert_danger("Failed to upload csv: {e$message}")
#         })
#       })
...
save_and_upload <- function(
  ...
  if (upload) {
    suppressWarnings({
      ...
      # Guarded per-file uploads for tag-level files (log errors, continue)
      # Use for loop instead of purrr::walk to avoid purrr 1.2.0 parameter wrapping issues
      for (f in pb_files) {
        if (!file.exists(f)) {
          # Skip silently or use cli::cli_alert_info if you want to see these messages
          next
        }
        
        # Extract scalars
        f_scalar <- as.character(f)[1]
        repo_scalar <- as.character(repo)[1]
        tag_scalar <- as.character(tag)[1]
        
        tryCatch({
          piggyback::pb_upload(f_scalar, repo = repo_scalar, tag = tag_scalar)
          cli::cli_alert_success("Uploaded {basename(f_scalar)} for tag {tag_scalar}")
        }, error = function(e) {
          cli::cli_warn("Tag-level upload failed for {basename(f_scalar)}: {conditionMessage(e)}")
        })
      }
    })

    # Per-season files - use for loop to avoid purrr 1.2.0 issues
    season_files <- list()
    for (season in seasons) {
      ...
      season_files[[length(season_files) + 1]] <- list(parquet = pq_path, rds = rds_path, csv = csv_path)
    }

    # Robust per-season upload with per-file checks and error handling
    # Use for loops instead of purrr::walk to avoid purrr 1.2.0 parameter wrapping issues
    for (filelist in season_files) {
      # Extract scalars outside the loop
      repo_scalar <- as.character(repo)[1]
      tag_scalar <- as.character(tag)[1]
      ...
      for (path in filelist) {
        ...
        path_scalar <- as.character(path)[1]

        tryCatch({
          # Upload without overwrite to avoid duplicates
          piggyback::pb_upload(path_scalar, repo = repo_scalar, tag = tag_scalar)
          cli::cli_alert_success("Uploaded {basename(path_scalar)} for tag {tag_scalar}")
        }, error = function(e) {
          cli::cli_warn("Upload failed for {basename(path_scalar)}: {conditionMessage(e)}; continuing")
        })
      }
...
#'
#' Calls save_and_upload(...) inside tryCatch and returns TRUE on success, FALSE on error.
#' This makes the calling scripts tolerant to unexpected upload errors in CI.
#'
#' @return Logical: TRUE if save_and_upload succeeded, FALSE otherwise (and logs a warning)
#' @export
#' @noRd
safe_save_and_upload <- function(...) {
  args <- list(...)
  tag_val <- if (!is.null(args$tag)) args$tag else NA_character_
  tryCatch({
    save_and_upload(...)
    TRUE
  }, error = function(e) {
    cli::cli_warn("save_and_upload failed for tag {tag_val}: {conditionMessage(e)}")
    FALSE
  })
}
