...
cat("\n=== Fitting Negative Binomial Team Strength Summary ===\n")
...
negbinom_tag <- "team_strength_negbinom_summary"
...
negbinom_files <- c(
  rds = file.path(tempdir(), paste0(negbinom_tag, "_", filter_season, ".rds")),
  parquet = file.path(
    tempdir(),
    paste0(negbinom_tag, "_", filter_season, ".parquet")
  ),
  arrow = file.path(
    tempdir(),
    paste0(negbinom_tag, "_", filter_season, ".arrow")
  )
)

saveRDS(nb_sum, negbinom_files[["rds"]])
arrow::write_parquet(nb_sum, negbinom_files[["parquet"]])
arrow::write_feather(nb_sum, negbinom_files[["arrow"]])
