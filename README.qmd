...
# Calculate dynamic x-axis limits based on the predicted distributions
min_y_pred <- quantile(
  pred_plot_df$y,
  probs = 0.001
) |>
...
# Add padding for logos and text
x_padding <- 5
x_min <- floor(min_y_pred / 5) * 5 - x_padding
x_max <- ceiling(max_y_pred / 5) * 5 + x_padding
...
score_dist_plot <- pred_plot_df |>
  ...
  # Team logos
  geom_nfl_logos(
    aes(x = x_min + 3, team_abbr = away_team),
    width = 0.045
  ) +
  geom_nfl_logos(
    aes(x = x_max - 3, team_abbr = home_team),
    width = 0.045
  ) +
  ...
  scale_x_continuous(
    breaks = seq(
      floor(min_y_pred / 10) * 10,
      ceiling(max_y_pred / 10) * 10,
      10
    ),
