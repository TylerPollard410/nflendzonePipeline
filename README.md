...
prediction_key <- paste(
  result_predict$game_id,
  result_predict$season,
  result_predict$week,
  result_predict$home_team,
  result_predict$away_team,
  sep = "|"
)
schedule_key <- paste(
  game_data$game_id,
...
schedule_idx <- match(prediction_key, schedule_key)

if (anyNA(schedule_idx)) {
  stop("Could not match all predicted games to schedule rows.")
}

pred_df <- result_predict |>
  mutate(
    spread_line = game_data$spread_line[schedule_idx],
    .after = away_team
  ) |>
  mutate(
